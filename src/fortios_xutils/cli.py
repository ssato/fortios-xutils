...
@click.option("-F", "--filetype", help="File type [json]")
def firewall_policy_save(filepaths, outdir, filetype=None):
    """
    ...
    \b
        ...
        > -O /tmp/1/ \\
        > tests/res/parsed/fortigate-01/all.json
        $ file /tmp/1/fortigate-01/
        ... firewall_policy_table.data.json.gz ...
        $
    ...
    """
    opts = dict(outdir=outdir, filetype=filetype)
    fortios_xutils.make_and_save_firewall_policy_tables(filepaths, **opts)
...
@click.option("-F", "--filetype", help="File type [json]")
def firewall_policy_search(filepath, ip_s, filetype=None):
    """
